...
/// <summary>
/// Extracts textual content from OpenDocument presentation files (ODP).
/// </summary>
internal sealed class OdpTextExtractor
{
    private const int MaxElements = 12_000;
    ...
    public ValueTask<string?> TryExtractAsync(Stream content, string mime, CancellationToken cancellationToken)
    {
        ...
        if (!string.Equals(mime, "application/vnd.oasis.opendocument.presentation", StringComparison.OrdinalIgnoreCase))
        {
        ...
        try
        {
            ...
            while (reader.Read())
            {
                ...
                {
                    if (IsSlideElement(reader))
                    {
                        EnsureParagraphBreak(builder, doubleBreak: true);
                    }
                    else if (IsParagraphElement(reader))
                    {
                        EnsureParagraphBreak(builder, doubleBreak: false);
                    }
    ...
    private static bool IsSlideElement(XmlReader reader)
    {
        return reader.NodeType == XmlNodeType.Element && reader.LocalName is "page";
    }

    private static bool IsParagraphElement(XmlReader reader)
    {
        return reader.NodeType == XmlNodeType.Element && reader.LocalName is "p" or "h";
    }
    ...
    private static void EnsureParagraphBreak(StringBuilder builder, bool doubleBreak)
    {
        if (builder.Length == 0)
        {
            return;
