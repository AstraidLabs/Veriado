    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                {
                    // norm�ln� ukon�en� workeru � ��dn� error
        ...
        {
            // fallback � kdyby n�co vylet�lo mimo vnit�n� try,
            // tady to rozli��me jako norm�ln� stop, ne jako chybu
            _logger.LogDebug("File system health check worker cancelled (outer catch).");
        }
        catch (TaskCanceledException) when (stoppingToken.IsCancellationRequested)
        {
            // pokud n�kdo hod� p��mo TaskCanceledException, po��d to bereme jako cancel
